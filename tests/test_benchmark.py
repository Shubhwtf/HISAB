...
class TestBenchmarkHarness:
    ...
    def test_baseline_a_deterministic_metrics(self, benchmark_dataset):
        ...
        assert res_a.recall < 0.60 # Rules-only misses complex anomalies
        assert res_a.double_loss_detected is False # Misses compounding double loss
    ...
    def test_baseline_b_naive_llm_metrics(self, benchmark_dataset):
        ...
        assert res_b.precision < 0.90 # Suffers from false positives
    ...
    def test_baseline_c_hisab_metrics(self, benchmark_dataset):
        ...
        assert res_c.precision == 1.0 # 100% precision
        assert res_c.recall == 1.0    # 100% recall
        assert res_c.f1_score == 1.0
        assert res_c.double_loss_detected is True
        assert res_c.double_loss_exposure_paise >= 14400000 # Full ₹144k detected
